...
import { singleUser, singleCollege, updateAccount } from '../../../store/action/index';
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    if (state.college) dispatch(singleCollege(state.college));
  }, [dispatch, id, state.college]);

  let editData = useSelector((state) => state.studentReducer.singleData)
  console.log(editData, " PRofile Data")

  if (!state.isEdit && editData !== false ) {
      const {
    firstName,
    lastName,
    birthdate,
